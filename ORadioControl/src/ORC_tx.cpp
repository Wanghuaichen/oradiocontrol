...
//  SPI_MasterWriteReg(CC2500_PKTLEN, sizeof(MessageData));
  SPI_MasterTransmit_void(CC2500_SFTX);       // Flush TX
...
void set_led(void)
{
  ...
  if(diff > 4)
  {
    ...
    if((led_count & 0xf) == 0)            // unteres Nibble 0
    {
      while(state.error)
      {
        led_count += 0x10;
...
void sendTelemetrie2UART(void)
{
  ...
  uartBuf[0] = b2hex(temp);
  uartBuf[1] = b2hex(((uint8_t )d1) / 0x10);
  uartBuf[2] = b2hex(d1);
  uartBuf[3] = ' ';
  temp = (d2 >> 8);
  uartBuf[4] = b2hex(temp / 0x10);
  uartBuf[5] = b2hex(temp);
  uartBuf[6] = b2hex(((uint8_t )d2) / 0x10);
  uartBuf[7] = b2hex(d2);
  uartBuf[8] = '\r';
  uartBuf[9] = '\n';
  uartBuf[10] = 0;
...
void chkFailSafe(void)
{
  if(checkKey())
  {
    ...
    SET_BIT(state.error, 3);
  }
  else
    RES_BIT(state.error, 3);
}
