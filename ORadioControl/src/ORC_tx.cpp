...
ISR(TIMER1_COMPA_vect) //2MHz pulse generation
{
  ...
  if( *pulsePtr == 0) {
    //currpulse=0;
    pulsePtr = pulses2MHz;
    pulsePol = 0;                 // Signal beginnt mit low (lowaktiv)
...
ISR(ADC_vect, ISR_NOBLOCK)
{
  ...
  uint8_t chan_temp = chan & 7;
  ADMUX = chan_temp | (1<<REFS0);      // Multiplexer frueh stellen kann nie schaden
  s_ana[chan_temp]   += ADC - s_ana[chan_temp] / 4; // Index ist immer 1 weniger als Kanal
  ++chan;
  if(chan & 0x20)
...
ISR(TIMER0_COMP_vect, ISR_NOBLOCK) //10ms timer
{
  ...
  setupPulses();        // aber nur wenn letzte Ausgabe fertig!!!!
  sei();
  OCR0 = OCR0 + 156;
  if(g_beepCnt){
    g_beepCnt--;
